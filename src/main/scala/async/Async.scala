...
object Async extends AsyncInterface {
  ...
  def transformSuccess(eventuallyX: Future[Int]): Future[Boolean] =
    {
      eventuallyX.map(computedNumber => computedNumber%2==0)
    }
  ...
  def sequenceComputations[A, B](
    ...
    makeAsyncComputation1.apply().flatMap {
      result1 => makeAsyncComputation2.apply().map(result2 => (result1, result2))
    }
  }
  ...
  def concurrentComputations[A, B](
    ...
    val eval1 = makeAsyncComputation1.apply()
    val eval2 = makeAsyncComputation2.apply()
    eval1.flatMap {
      result1 => eval2.map(result2 =>(result1, result2))
    }
  }
      ...
      val futureApi = new FutureBasedApi {
        ...
        def computeIntAsync(): Future[Int] = {
          callbackBasedApi.computeIntAsync(
            callBackResult => {
              p.tryComplete(callBackResult) //success or catches throwable
            }
          )
